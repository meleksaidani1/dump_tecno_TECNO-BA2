AUTO_ADD_GLOBAL_DEFINE_BY_NAME = MTK_MAGICONFERENCE_SUPPORT TRAN_LCM_SUPPORT MTK_HAC_SUPPORT CUSTOM_KERNEL_PDR_SENSOR MTK_EMBMS_SUPPORT MTK_SIM_LOCK_POWER_ON_WRITE_PROTECT MTK_CROSSMOUNT_MULTI_CH_SUPPORT MTK_MODEM_MONITOR_SUPPORT MTK_BT_BLUEDROID_AVRCP_TG_16 MTK_CROSSMOUNT_SUPPORT CUSTOM_KERNEL_ANSWER_CALL_SENSOR MTK_CAM_STEREO_DENOISE_SUPPORT CUSTOM_KERNEL_UNCALI_MAG_SENSOR CUSTOM_KERNEL_UNCALI_GYRO_SENSOR MTK_BLULIGHT_DEFENDER_SUPPORT MTK_LCM_DEVICE_TREE_SUPPORT MTK_BT_BLUEDROID_A2DP_APTX MTK_NVCONFIG_PARTITION_SUPPORT MTK_BT_BLUEDROID_HFP_AG_17 MTK_AURISYS_PHONE_CALL_SUPPORT MTK_HEADSET_ACTIVE_NOISE_CANCELLATION MTK_DYNAMIC_SBP_SUPPORT MTK_FACTORY_GAMMA_SUPPORT MTK_VT3G324M_SUPPORT MTK_WFC_SUPPORT MTK_BT_BLUEDROID_AVRCP_TG_15 MTK_BT_BLUEDROID_PLUS MTK_CAM_VSDOF_SUPPORT MTK_EXTERNAL_SIM_SUPPORT MTK_EFUSE_WRITER_SUPPORT MTK_FACTORY_MODE_IN_GB2312 MTK_A1_FEATURE MTK_GMO_ROM_OPTIMIZE MTK_GMO_RAM_OPTIMIZE SENSOR_BATCH_SUPPORT MTK_LOSSLESS_BT_SUPPORT MTK_DTAG_DUAL_APN_SUPPORT MTK_DEINTERLACE_SUPPORT MTK_JEITA_STANDARD_SUPPORT MTK_AUDIO_ALAC_SUPPORT MTK_BQ24261_SUPPORT MTK_REGIONALPHONE_SUPPORT MTK_VIDEO_VP8ENC_SUPPORT LENOVO_X2 MTK_BQ24250_SUPPORT X2_BQ27531_SUPPORT X2_CHARGING_STANDARD_SUPPORT MTK_MAX77819_SUPPORT MTK_PCA9575A_SUPPORT MTK_MD_LOW_BAT_SUPPORT MTK_ALPS_BOX_TVE_SUPPORT MTK_EXTMD_NATIVE_DOWNLOAD_SUPPORT MTK_ALPS_BOX_DONGLE_SUPPORT MTK_BESSURROUND_SUPPORT MTK_BOX_AUDIO_DDPLUS_SUPPORT MTK_PLAYBACK_60FPS_TO_30FPS MTK_CAM_MMSDK_SUPPORT MTK_ANT_SUPPORT MTK_MIRAVISION_SETTING_SUPPORT MTK_PUMP_EXPRESS_SUPPORT MTK_BQ24157_SUPPORT MTK_MPEG_DASH_SUPPORT MTK_VOLTE_SUPPORT MTK_IMS_SUPPORT MTK_VOW_SUPPORT MTK_IPTV_SUPPORT MTK_PASSPOINT_R2_SUPPORT MTK_EXTERNAL_DONGLE_SUPPORT MTK_ALPS_BOX_SUPPORT MTK_OD_SUPPORT MTK_BQ24296_SUPPORT MTK_CAM_HDR_SUPPORT MTK_TC1_FEATURE MTK_BATTERY_PROTECT MTK_WIRELESS_CHARGER_SUPPORT MTK_CAM_STEREO_CAMERA_SUPPORT MTK_BESLOUDNESS_SUPPORT MTK_CAM_VHDR_SUPPORT MTK_CAM_EIS_SUPPORT MTK_CAM_VIDEO_FACEBEAUTY_SUPPORT MTK_CAM_FACEBEAUTY_SUPPORT MTK_WFD_SINK_UIBC_SUPPORT MTK_WFD_SINK_SUPPORT MTK_CAM_VSS_SUPPORT MTK_CAM_ZSD_SUPPORT MTK_POWER_EXT_DETECT MTK_CAM_LOMO_SUPPORT MTK_POST_PROCESS_FRAMEWORK_SUPPORT MTK_CAM_DEPTH_AF_SUPPORT MTK_MERGE_INTERFACE_SUPPORT MTK_CAM_NATIVE_PIP_SUPPORT MTK_CAM_NR3D_SUPPORT MTK_CAM_SINGLE_3DSHOT_SUPPORT MTK_CAM_AUTORAMA_SUPPORT MTK_CAM_IMAGE_REFOCUS_SUPPORT MTK_CAM_ASD_SUPPORT MTK_CAM_FD_SUPPORT MTK_MLC_NAND_SUPPORT MTK_FORCE_CLUSTER1 TRUSTONIC_TEE_SUPPORT MTK_RAWFS_NAND_SUPPORT MTK_LOMO_SUPPORT MTK_EXTERNAL_LDO MTK_PMIC_MT6397 MTK_TOUCH_BOOST MTK_SLOW_MOTION_VIDEO_SUPPORT MTK_VIDEO_HEVC_SUPPORT MTK_AUTO_DETECT_ALSPS MTK_GPT_SCHEME_SUPPORT MT6280_SUPER_DONGLE MTK_PRIVACY_PROTECTION_LOCK MTK_SIM_RECOVERY MTK_FAN5402_SUPPORT MTK_AUDIO_HD_REC_SUPPORT MTK_AUDIO MTK_CLEARMOTION_SUPPORT MTK_FSCK_TUNE MTK_NEW_COMBO_EMMC_SUPPORT MTK_PERSIST_PARTITION_SUPPORT MTK_DX_HDCP_SUPPORT MTK_COMBO_NAND_SUPPORT MTK_SMARTBOOK_SUPPORT MTK_DFO_RESOLUTION_SUPPORT MTK_HIGH_RESOLUTION_AUDIO_SUPPORT MTK_AUDIO_EXTCODEC_SUPPORT MTK_SUBTITLE_SUPPORT MTK_SDIOAUTOK_SUPPORT MTK_MSDC1_NOT_SUPPORT_SDR104 MTK_FEMTO_CELL_SUPPORT MTK_CHIPTEST_INT MTK_USER_ROOT_SWITCH MTK_ENABLE_MD5 MTK_WMV_PLAYBACK_SUPPORT MTK_WMA_PLAYBACK_SUPPORT MTK_MOBILE_MANAGEMENT MTK_PARTITION_TABLE_PLAIN_TEXT MTK_BQ24156_SUPPORT CUSTOM_KERNEL_BAROMETER MTK_BQ24158_SUPPORT MTK_TABLET_PLUGIN_BUILD MTK_MULTI_PARTITION_MOUNT_ONLY_SUPPORT MTK_BQ24297_SUPPORT MTK_PM_STATS_SUPPORT MTK_SEC_WFD_VIDEO_PATH_SUPPORT MTK_BATLOWV_NO_PANEL_ON_EARLY MTK_SIM_HOT_SWAP_COMMON_SLOT MTK_TER_SERVICE MTK_OWNER_SDCARD_ONLY_SUPPORT MTK_DRM_PLAYREADY_SUPPORT MTK_SHOW_MSENSOR_TOAST_SUPPORT MTK_AUDIO_CHANGE_SUPPORT IS_VCORE_USE_6333VCORE IS_VRF18_USE_6333VRF18 IS_VM_USE_6333VM MTK_VIDEO_AUTO_FLASH_SUPPORT MTK_AIV_SUPPORT MTK_WFD_HDCP_TX_SUPPORT PURE_AP_USE_EXTERNAL_MODEM MTK_NO_NEED_USB_LED MTK_HDMI_HDCP_SUPPORT MTK_INTERNAL_HDMI_SUPPORT MTK_INTERNAL_MHL_SUPPORT MTK_DMNR_TUNING_AT_MD NAND_OTP_SUPPORT MTK_DVFS_DISABLE_LOW_VOLTAGE_SUPPORT MTK_VOIP_ENHANCEMENT_SUPPORT MTK_HANDSFREE_DMNR_SUPPORT MTK_SWCHR_SUPPORT MTK_MT6333_SUPPORT MTK_DRM_KEY_MNG_SUPPORT MTK_SEC_VIDEO_PATH_SUPPORT MTK_WVDRM_L1_SUPPORT MTK_IN_HOUSE_TEE_SUPPORT MTK_DISABLE_POWER_ON_OFF_VOLTAGE_LIMITATION MTK_CTA_SUPPORT MTK_DFO_SUPPORT MTK_BIP_SCWS MTK_MDM_FUMO MTK_MDM_LAWMO MTK_MDM_SCOMO MTK_UART_USB_SWITCH MTK_FAT_ON_NAND MTK_AUDIO_RAW_SUPPORT MTK_MD_SHUT_DOWN_NT MTK_CACHE_MERGE_SUPPORT MTK_IPV6_TETHER_PD_MODE MTK_YMAL_SCATTER_FILE_SUPPORT MTK_RADIOOFF_POWER_OFF_MD MTK_MT8193_HDCP_SUPPORT MTK_PLATFORM_OPTIMIZE MTK_HW_ENHANCE MTK_PQ_SUPPORT MTK_AUDIO_ADPCM_SUPPORT HAVE_ADPCMENCODE_FEATURE MTK_2IN1_SPK_SUPPORT MTK_BQ27541_SUPPORT MTK_WFD_SUPPORT MTK_IPV6_TETHER_NDP_MODE MTK_TETHERING_EEM_SUPPORT MTK_AAL_SUPPORT MTK_KERNEL_POWER_OFF_CHARGING MTK_FLIGHT_MODE_POWER_OFF_MD MTK_MMPROFILE_SUPPORT MTK_GPS_SUPPORT MTK_MULTIBRIDGE_SUPPORT MTK_CAMCORDER_PROFILE_MID_MP4 MTK_FM_SUPPORT MTK_TABLET_PLATFORM MTK_AUTO_DETECT_MAGNETOMETER MTK_NCP1851_SUPPORT MTK_NFC_SUPPORT MTK_MASS_STORAGE MTK_COMBO_QUICK_SLEEP_SUPPORT MTK_MT8193_SUPPORT HAVE_AACENCODE_FEATURE MTK_FM_50KHZ_SUPPORT MTK_COMBO_SUPPORT CUSTOM_KERNEL_GYROSCOPE MTK_SHARED_SDCARD MTK_EMMC_DISCARD MTK_DSPIRDBG MTK_MULTI_STORAGE_SUPPORT CUSTOM_KERNEL_ALSPS MTK_ENABLE_VIDEO_EDITOR CUSTOM_KERNEL_ACCELEROMETER MTK_DUAL_MIC_SUPPORT MTK_WAPI_SUPPORT MTK_FD_SUPPORT MTK_DISPLAY_HIGH_RESOLUTION MTK_BT_SUPPORT MTK_TABLET_DRAM MTK_FACEBEAUTY_SUPPORT MTK_FAN5405_SUPPORT MTK_BQ24160_SUPPORT MTK_HIGH_QUALITY_THUMBNAIL MTK_SPH_EHN_CTRL_SUPPORT MTK_FM_TX_SUPPORT MTK_BRAZIL_CUSTOMIZATION_VIVO MTK_BRAZIL_CUSTOMIZATION_CLARO MTK_IPV6_SUPPORT MTK_AUTORAMA_SUPPORT MTK_EAP_SIM_AKA MTK_BSP_PACKAGE MTK_BRAZIL_CUSTOMIZATION MTK_NVRAM_SECURITY CUSTOM_KERNEL_MAGNETOMETER MTK_WLANBT_SINGLEANT MTK_WLAN_SUPPORT MTK_TETHERINGIPV6_SUPPORT MTK_AUTO_DETECT_ACCELEROMETER HAVE_CMMB_FEATURE MTK_MT8193_HDMI_SUPPORT MTK_EMMC_SUPPORT_OTP MTK_PRODUCT_INFO_SUPPORT MTK_FM_SHORT_ANTENNA_SUPPORT MTK_HDMI_SUPPORT MTK_M4U_SUPPORT MTK_MT8193_NFI_SUPPORT MTK_ION_SUPPORT MTK_FSCK_MSDOS_MTK MTK_BQ24196_SUPPORT MTK_FM_RX_SUPPORT MTK_WB_SPEECH_SUPPORT ENCRY_PARTITION_SUPPORT MTK_SD_REINIT_SUPPORT MTK_SENSOR_SUPPORT MTK_EMMC_SUPPORT MTK_COMBO_CORE_DUMP_SUPPORT MTK_DHCPV6C_WIFI MTK_EMULATOR_SUPPORT MTK_NAND_UBIFS_SUPPORT MTK_YAML_SCATTER_FILE_SUPPORT MTK_VIDEO_VP9_SUPPORT MTK_THERMAL_PA_VIA_ATCMD MTK_FLV_PLAYBACK_SUPPORT MTK_AUDIO_APE_SUPPORT MTK_AVI_PLAYBACK_SUPPORT USE_FRAUNHOFER_AAC MTK_DRM_APP MTK_OGM_PLAYBACK_SUPPORT MTK_MTKPS_PLAYBACK_SUPPORT MTK_MT6306_SUPPORT TRAN_CAMERA_FEATURE_V1 TRAN_CAMERA_ARCSOFT_BEAUTY TRAN_FLASH_SLIDE_CONTROL TRAN_CUSTOM_DARK_COLOR TRAN_CAMERA_FRONT_MIRROR TRAN_CAMERA_VISIDON_FACEBEAUTY TRAN_CAMERA_ARCSOFT_SINGLE_FRM_DENOISE TRAN_CAMERA_ARCSOFT_PANORAMA TRAN_CAMERA_ARCSOFT_PICSELFIE TRAN_CAMERA_ARCSOFT_LOWLIGHT TRAN_CAMERA_ARCSOFT_NIGHTHAWK TRAN_CAMERA_VISIDON_ASD TRAN_CAMERA_SENSETIME_SINGLEBLUR TRAN_CAMERA_WATERMARK TRAN_CAMERA_FEATURE_FACE_ID
AUTO_ADD_GLOBAL_DEFINE_BY_NAME_VALUE = MTK_FUSED_LOCATION_SUPPORT MTK_NFC_PACKAGE MTK_CAM_STEREO_CAMERA_FACING MTK_JPEG_HW_RESIZER_TYPE MAXIM_SPEAKER_SUPPORT MTK_CAM_ZSDHDR_SUPPORT MTK_CAM_ZSDMFB_SUPPORT SIM_ME_LOCK_MODE CUSTOM_CONFIG_MAX_DRAM_SIZE MTK_MD_SBP_CUSTOM_VALUE MTK_CAM_MFB_SUPPORT MTK_CAM_CONTINUOUS_SHOT_MODE MTK_VIDEO_4KH264_SUPPORT MTK_TELEPHONY_BOOTUP_MODE_SLOT2 MTK_TELEPHONY_BOOTUP_MODE_SLOT1 MTK_NFC_SE_NUM MTK_OVERLAY_ENGINE_SUPPORT MTK_SIM1_SOCKET_TYPE MTK_SIM2_SOCKET_TYPE CUSTOM_KERNEL_MAIN2_IMGSENSOR CUSTOM_KERNEL_SSW MTK_LCM_PHYSICAL_ROTATION LCM_WIDTH MTK_SINGLE_3DSHOT_SUPPORT MTK_EXTERNAL_MODEM_SLOT MTK_NEON_SUPPORT LCM_HEIGHT MTK_NTFS_OPENSOURCE_SUPPORT
AUTO_ADD_GLOBAL_DEFINE_BY_VALUE = CUSTOM_TRAN_PROJECT CUSTOM_TRAN_BOM CUSTOM_HAL_LENS BOOT_LOGO MTK_AUDIO_BLOUD_CUSTOMPARAMETER_REV MTK_PLATFORM CUSTOM_KERNEL_LENS CUSTOM_KERNEL_MAIN_BACKUP_LENS CUSTOM_KERNEL_MAIN_IMGSENSOR MTK_BT_CHIP MTK_WLAN_CHIP CUSTOM_KERNEL_SUB_BACKUP_IMGSENSOR CUSTOM_KERNEL_MAIN_BACKUP_IMGSENSOR CUSTOM_KERNEL_FLASHLIGHT CUSTOM_KERNEL_SUB_IMGSENSOR CUSTOM_KERNEL_SUB_LENS CUSTOM_KERNEL_IMGSENSOR MTK_FM_RX_AUDIO MTK_COMBO_CHIP MTK_GPS_CHIP CUSTOM_KERNEL_SUB_BACKUP_LENS CUSTOM_KERNEL_MAIN_LENS MTK_FM_TX_AUDIO MTK_FM_CHIP CUSTOM_KERNEL_MAIN2_IMGSENSOR BOOT_LOGO
#BSP:modify BAEGHBLEQ-58 by zhijun.ye 20190306 start
BOOT_LOGO = fwvgaplus
#BSP:modify BAEGHBLEQ-58 by zhijun.ye 20190306 end
BUILD_KERNEL = yes
BUILD_LK = yes
BUILD_MD32 = no
BUILD_MTK_SDK =
BUILD_PRELOADER = yes
BUILD_UBOOT = no
CUSTOM_BUILD_VERNO = CY-BA2-H8027-U
CUSTOM_CONFIG_MAX_DRAM_SIZE = 0x40000000
CUSTOM_HAL_ANT = mt6752_ant_m1
CUSTOM_HAL_AUDIOFLINGER = audio
CUSTOM_HAL_BLUETOOTH = bluetooth
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 Start
TRAN_CUSTOM_DARK_COLOR = yes
CUSTOM_HAL_CAMERA = camera
CUSTOM_HAL_CAM_CAL =
CUSTOM_HAL_COMBO = mt6580
CUSTOM_HAL_FLASHLIGHT = dummy_flashlight
CUSTOM_HAL_IMGSENSOR = gc8034main_mipi_raw gc5025_mipi_raw gc8034_mipi_raw gc8034mainb_mipi_raw gc5025b_mipi_raw gc8034b_mipi_raw
CUSTOM_HAL_LENS = dw9714af dummy_lens
CUSTOM_HAL_MAIN2_IMGSENSOR =
CUSTOM_HAL_MAIN_BACKUP_IMGSENSOR =
CUSTOM_HAL_MAIN_BACKUP_LENS = dummy_lens
CUSTOM_HAL_MAIN_IMGSENSOR = gc8034main_mipi_raw gc5025_mipi_raw gc8034mainb_mipi_raw gc5025b_mipi_raw
CUSTOM_HAL_MAIN_LENS = dw9714af
CUSTOM_HAL_MSENSORLIB =akl
CUSTOM_HAL_SENSORS = sensor
CUSTOM_HAL_SUB_BACKUP_IMGSENSOR =
CUSTOM_HAL_SUB_BACKUP_LENS = dummy_lens
CUSTOM_HAL_SUB_IMGSENSOR = gc8034_mipi_raw gc8034b_mipi_raw
CUSTOM_HAL_SUB_LENS = dummy_lens
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 End
CUSTOM_HIFI_SENSORS = no
CUSTOM_KERNEL_ACCELEROMETER = yes
CUSTOM_KERNEL_ALSPS = yes
CUSTOM_KERNEL_ANSWER_CALL_SENSOR =
CUSTOM_KERNEL_BAROMETER =
CUSTOM_KERNEL_BATTERY = battery
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 Start
CUSTOM_KERNEL_CAMERA = camera
CUSTOM_KERNEL_CAM_CAL =
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 End
#ITD:ADD CAMERA_3RD zongxin.WB 180820 Start
TRAN_CAMERA_FEATURE_V1 = yes
TRAN_CAMERA_ARCSOFT_BEAUTY = yes
TRAN_CAMERA_FRONT_MIRROR = yes
TRAN_CAMERA_VISIDON_FACEBEAUTY = no
TRAN_CAMERA_ARCSOFT_SINGLE_FRM_DENOISE = no
TRAN_CAMERA_ARCSOFT_PANORAMA = no
TRAN_CAMERA_ARCSOFT_PICSELFIE = yes
TRAN_CAMERA_ARCSOFT_LOWLIGHT = yes
TRAN_CAMERA_ARCSOFT_NIGHTHAWK = yes
TRAN_CAMERA_VISIDON_ASD = no
TRAN_CAMERA_SENSETIME_SINGLEBLUR = no
TRAN_CAMERA_WATERMARK = yes
#ITD:ADD CAMERA_3RD zongxin.WB 180820 End
CUSTOM_KERNEL_CORE = src
CUSTOM_KERNEL_DCT = dct
CUSTOM_KERNEL_DEVICE_ORIENTATION =
CUSTOM_KERNEL_FLASHLIGHT = constant_flashlight
TRAN_FLASH_SLIDE_CONTROL = yes
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 start
CUSTOM_KERNEL_GYROSCOPE = no
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 end
CUSTOM_KERNEL_HDMI =
CUSTOM_KERNEL_HEADSET = accdet
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 Start
CUSTOM_KERNEL_IMGSENSOR = gc8034main_mipi_raw gc5025_mipi_raw gc8034_mipi_raw
CUSTOM_KERNEL_KPD = kpd
CUSTOM_KERNEL_LEDS = mt65xx
CUSTOM_KERNEL_LENS = dw9714af dummy_lens
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 End
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 start
CUSTOM_KERNEL_MAGNETOMETER = no
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 end
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 Start
CUSTOM_KERNEL_MAIN2_BACKUP_IMGSENSOR =
CUSTOM_KERNEL_MAIN2_IMGSENSOR =
CUSTOM_KERNEL_MAIN_BACKUP_IMGSENSOR =
CUSTOM_KERNEL_MAIN_BACKUP_LENS = dummy_lens
CUSTOM_KERNEL_MAIN_IMGSENSOR = gc8034main_mipi_raw gc5025_mipi_raw
CUSTOM_KERNEL_MAIN_LENS = dw9714af
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 End
CUSTOM_KERNEL_MOTION_DETECT =
CUSTOM_KERNEL_ORIENTATION_SENSOR = no
CUSTOM_KERNEL_PDR_SENSOR = no
CUSTOM_KERNEL_RTC = rtc
CUSTOM_KERNEL_SOUND = amp_6323pmic_spk
CUSTOM_KERNEL_SSW =
CUSTOM_KERNEL_STATIONARY_SENSOR =
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 Start
CUSTOM_KERNEL_SUB_BACKUP_IMGSENSOR =
CUSTOM_KERNEL_SUB_BACKUP_LENS = dummy_lens
CUSTOM_KERNEL_SUB_IMGSENSOR = gc8034_mipi_raw
#ITD:Modify XLLJHBLEW-12 tianpeng.zhang 180801 End
CUSTOM_KERNEL_SUB_LENS = dummy_lens
#ITD:Modify KBQJHBLEW-2 By jie.liu 20180910 End
CUSTOM_KERNEL_TOUCHPANEL = GT1XX
CUSTOM_KERNEL_UNCALI_GYRO_SENSOR =
CUSTOM_KERNEL_UNCALI_MAG_SENSOR =
CUSTOM_KERNEL_USB = mt6577
CUSTOM_KERNEL_VIBRATOR = vibrator
#TSD: BAEGHBLEQ-5 modify by zhiqing.deng 20190424 start
#TSD: BAEGHBLEQ-5 modify by xiaochen.ye 20190219 start
CUSTOM_MODEM = ba2_h8027_drdi ba2_h8027_drdi_fulllink
#TSD: BAEGHBLEQ-5 modify by xiaochen.ye 20190219 end
#TSD: BAEGHBLEQ-5 modify by zhiqing.deng 20190424 end
CUSTOM_SEC_AUTH_SUPPORT = no
CUSTOM_SEC_SIGNTOOL_SUPPORT = no
DCORE_TEST_MODE1 = no
DISABLE_EARPIECE = no
ENCRY_PARTITION_SUPPORT = no
EVB = no
EVDO_IR_SUPPORT = no
FEATURE_FTM_AUDIO_AUTOTEST = yes
FEATURE_FTM_AUDIO_TEST = yes
FPGA_EARLY_PORTING = no
GOOGLE_RELEASE_RIL = no
HAVE_AACENCODE_FEATURE = yes
HAVE_ADPCMENCODE_FEATURE = no
HAVE_APPC_FEATURE = no
HAVE_VORBISENC_FEATURE = yes
IS_VCORE_USE_6333VCORE = no
IS_VM_USE_6333VM = no
IS_VRF18_USE_6333VRF18 = no
LCM_FAKE_HEIGHT = 0
LCM_FAKE_WIDTH = 0
#BSP:modify BEFBLYQ-752 by cheng.chen1 20190301 strat
LCM_HEIGHT = 960
LCM_WIDTH = 480
#BSP:modify BEFBLYQ-752 by cheng.chen1 20190301 end
TRAN_LCM_SUPPORT = yes
LIGHT_NVRAM_DAEMON = no
LINUX_KERNEL_VERSION = kernel-3.18
LOVELYFONTS_ICON_SHOW = no
#APP: modify KBQJHBLEW-10 by hanxin.zhang 2018/9/25 start
LOVELYFONTS_SUPPORT = no
#APP: modify KBQJHBLEW-10 by hanxin.zhang 2018/9/25 end
MAXIM_SPEAKER_SUPPORT = no
MD32_TOOLCHAIN_MODULE = IPDesigner/12R2.36-64
MD5_SIZE = 0x01600000
MD5_SMEM_SIZE = 0x00200000
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 start
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 end
MNTL_SUPPORT = no
MT6280_SUPER_DONGLE = no
MTK_2IN1_SPK_SUPPORT = no
MTK_AAL_SUPPORT = yes
MTK_ACAO_SUPPORT = no
MTK_ACDK_BUILD_DUMMY_ENG = no
MTK_AEE_SUPPORT = yes
MTK_AGPS_APP = yes
MTK_AIV_SUPPORT = no
MTK_ALARM_AWARE_UPLINK_SUPPORT = no
MTK_ALPS_BOX_DONGLE_SUPPORT = no
MTK_ALPS_BOX_SUPPORT = no
MTK_ALPS_BOX_TVE_SUPPORT = no
MTK_ALPS_GATEWAY_AP_ROUTER_SUPPORT = no
MTK_ALPS_GATEWAY_SUPPORT = yes
MTK_ANDROID_LOG_MUCH_COUNT = 4000
MTK_ANT_SUPPORT = no
MTK_AOD_SUPPORT = no
MTK_APKINSTALLER_APP = no
MTK_APPENDED_DTB_SUPPORT = yes
MTK_APP_GUIDE = yes
MTK_AP_SPEECH_ENHANCEMENT = no
MTK_ASR_SUPPORT = no
MTK_AUDIO = yes
MTK_AUDIO_ADPCM_SUPPORT = yes
MTK_AUDIO_ALAC_SUPPORT = yes
MTK_AUDIO_APE_SUPPORT = yes
MTK_AUDIO_BLOUD_CUSTOMPARAMETER_REV = MTK_AUDIO_BLOUD_CUSTOMPARAMETER_V5
MTK_AUDIO_CHANGE_SUPPORT = no
MTK_AUDIO_CODEC_SUPPORT_TABLET = no
MTK_AUDIO_EXTCODEC_SUPPORT = no
MTK_AUDIO_GAIN_TABLE_SUPPORT = no
MTK_AUDIO_HD_REC_SUPPORT = yes
MTK_AUDIO_HYBRID_NLE_SUPPORT = no
MTK_AUDIO_MIC_INVERSE = no
MTK_AUDIO_NUMBER_OF_MIC = 1
MTK_AUDIO_NUMBER_OF_SPEAKER = 1
MTK_AUDIO_RAW_SUPPORT = yes
#BSP:modify KALHBLEW-2 by haiping.bai 20180719 start
MTK_AUDIO_SPEAKER_PATH = int_hp_buf
#BSP:modify KALHBLEW-2 by haiping.bai 20180719 end
MTK_AUDIO_TUNNELING_SUPPORT = no
MTK_AUIDO_MIC_INVERSE = no
MTK_AURISYS_FRAMEWORK_SUPPORT = no
MTK_AURISYS_PHONE_CALL_SUPPORT = no
MTK_AUTOIP_SUPPORT = no
MTK_AUTORAMA_SUPPORT = yes
MTK_AUTO_DETECT_ACCELEROMETER = no
MTK_AUTO_DETECT_ALSPS = no
MTK_AUTO_DETECT_MAGNETOMETER = no
MTK_AUTO_SANITY = no
MTK_AVI_PLAYBACK_SUPPORT = yes
MTK_BATLOWV_NO_PANEL_ON_EARLY = no
MTK_BATTERY_PROTECT = no
MTK_BESLOUDNESS_RUN_WITH_HAL = no
MTK_BESLOUDNESS_SUPPORT = yes
MTK_BESSURROUND_SUPPORT = no
MTK_BG_POWER_SAVING_SUPPORT = yes
MTK_BG_POWER_SAVING_UI_SUPPORT = yes
MTK_BIP_SCWS = yes
MTK_BLULIGHT_DEFENDER_SUPPORT = no
MTK_BOX_AUDIO_DDPLUS_SUPPORT = no
MTK_BOX_CAM_SUPPORT = no
MTK_BQ24156_SUPPORT = no
MTK_BQ24157_SUPPORT = no
MTK_BQ24158_SUPPORT = no
MTK_BQ24160_SUPPORT = no
MTK_BQ24261_SUPPORT = no
MTK_BQ24296_SUPPORT = no
MTK_BQ24297_SUPPORT = no
MTK_BQ27541_SUPPORT = no
MTK_BRANCH = alps-mp-o1.mp2
MTK_BRAZIL_CUSTOMIZATION = no
MTK_BRAZIL_CUSTOMIZATION_CLARO = no
MTK_BRAZIL_CUSTOMIZATION_TIM = no
MTK_BRAZIL_CUSTOMIZATION_VIVO = no
MTK_BSP_PACKAGE = yes
MTK_BTLOGGER_SUPPORT = yes
MTK_BT_BLUEDROID_A2DP_APTX = no
MTK_BT_BLUEDROID_AVRCP_TG_15 = no
MTK_BT_BLUEDROID_AVRCP_TG_16 = no
MTK_BT_BLUEDROID_DUN_GW_12 = no
MTK_BT_BLUEDROID_HFP_AG_17 = no
MTK_BT_BLUEDROID_PLUS = yes
MTK_BT_CHIP = MTK_CONSYS_MT6580
MTK_BT_CUSTOM = no
MTK_BT_HOST_SNOOP_LOG_SUPPORT = yes
MTK_BT_INTEROP_EXTENSION = yes
MTK_BT_SUPPORT = yes
MTK_BT_WIFI_COEX_RADIO_MODIFY = no
MTK_BUFFER_COMPRESSION_SUPPORT = no
MTK_BUILD_VERNO=BA2-H8027ABC-OGo-210319V543
FINGERPRINT_BUILD_VERNO=ABC-210319V543
MTK_BWC_SUPPORT = yes
MTK_C2K_LTE_MODE = 0
MTK_CACHE_MERGE_SUPPORT = no
MTK_CALENDAR_IMPORTER_APP = yes
MTK_CAMCORDER_PROFILE_MID_MP4 = no
MTK_CAMERA_APP = no
#CameraAP:add CAMERAEYL-288 shiyang.zhang 2018-08-14 start
MTK_CAMERA_APP_VERSION = tecnocamera2
#CameraAP:add CAMERAEYL-288 shiyang.zhang 2018-08-14 end
#CDD: add KBQJHBLEW-293 by tao.yang 20181018 start
HIOS_CAMERA_USE_GOOGLE_PHOTOS_SUPPORT = yes
#CDD: add KBQJHBLEW-293 by tao.yang 20181018 end
#CDD: add KBQJHBLEW-1121 by qizhe.feng 20190213 start
TECNOCAM_NIGHTSCENEPICTURESIZEDOWN = yes
#CDD: add KBQJHBLEW-1121 by qizhe.feng 20190213 end
MTK_CAM_ADV_CAM_SUPPORT = no
MTK_CAM_ASD_SUPPORT = yes
MTK_CAM_AUTORAMA_SUPPORT = yes
MTK_CAM_CONTINUOUS_SHOT_MODE = 1
MTK_CAM_DEPTH_AF_SUPPORT = no
MTK_CAM_DUALPD_SUPPORT = no
MTK_CAM_DUAL_ZOOM_FUSION_SUPPORT = no
MTK_CAM_DUAL_ZOOM_MTK_FOV_SUPPORT = no
MTK_CAM_DUAL_ZOOM_SUPPORT = no
MTK_CAM_DUAL_ZOOM_VENDOR_FOV_SUPPORT = no
#ITD: modify by pengtao 20190116 start
MTK_CAM_EIS_SUPPORT = no
TRAN_CAMERA_FEATURE_FACE_ID = yes
#ITD: modify by pengtao 20190116 end
MTK_CAM_FACEBEAUTY_SUPPORT = yes
MTK_CAM_FD_SUPPORT = yes
MTK_CAM_HDR_SUPPORT = yes
MTK_CAM_IMAGE_REFOCUS_SUPPORT = NONE
MTK_CAM_LOMO_SUPPORT = no
MTK_CAM_MFB_BUILDIN_SUPPORT = 0
MTK_CAM_MFB_SUPPORT = 0
MTK_CAM_MMSDK_SUPPORT = no
MTK_CAM_NATIVE_PIP_SUPPORT = no
MTK_CAM_NR3D_SUPPORT = no
MTK_CAM_PDO_TEST_ENABLE = no
MTK_CAM_SINGLE_3DSHOT_SUPPORT = no
MTK_CAM_STEREO_CAMERA_FACING = rear
MTK_CAM_STEREO_CAMERA_SUPPORT = no
MTK_CAM_STEREO_DENOISE_SUPPORT = no
MTK_CAM_SW_VERSION = ver1
MTK_CAM_USER_DBG_LOG_OFF = no
MTK_CAM_VHDR_SUPPORT = no
MTK_CAM_VIDEO_FACEBEAUTY_SUPPORT = no
MTK_CAM_VSDOF_SUPPORT = no
MTK_CAM_VSS_SUPPORT = yes
MTK_CAM_ZSDHDR_SUPPORT = no
MTK_CAM_ZSDMFB_SUPPORT = no
MTK_CAM_ZSD_SUPPORT = yes
MTK_CARRIEREXPRESS_APK_INSTALL_SUPPORT = no
MTK_CARRIEREXPRESS_SWITCH_MODE = 1
MTK_CC33_SUPPORT = no
MTK_CDS_EM_SUPPORT = yes
MTK_CHAMELEON_DISPLAY_SUPPORT = no
MTK_CHARM_SUPPORT = no
MTK_CHIPTEST_INT = no
MTK_CHIP_VER = S01
MTK_CHIP_VERSION = 0
MTK_CIP_SUPPORT = no
MTK_CLEARMOTION_SUPPORT = no
MTK_CMAS_SUPPORT = no
MTK_CMCC_RCS_QRCODE_SUPPORT = no
MTK_COMBO_CHIP = CONSYS_6580
MTK_COMBO_NAND_SUPPORT = no
MTK_COMBO_SUPPORT = yes
MTK_CONSUMER_PARTIAL_UPDATE_SUPPORT = no
MTK_CPU = arm_cortexa7
MTK_CROSSMOUNT_MULTI_CH_SUPPORT = no
MTK_CROSSMOUNT_SUPPORT = no
#SID1:modify BAEGHBLEQ-61 by weike.xu 20190311 start
MTK_CTA_SET = no
#SID1:modify BAEGHBLEQ-61 by weike.xu 20190311 end
MTK_CTA_SUPPORT = no
MTK_CTM_SUPPORT = no
MTK_CT_VOLTE_SUPPORT = no
MTK_CUSTOMERSERVICE_APP = no
MTK_CUSTOM_USERLOAD_ENGINEERMODE = no
MTK_DATADIALOG_APP = no
MTK_DEFAULT_DATA_OFF = no
MTK_DEINTERLACE_SUPPORT = no
MTK_DEVREG_APP = no
MTK_DFO_RESOLUTION_SUPPORT = no
MTK_DHCPV6C_WIFI = yes
MTK_DIGITAL_MIC_SUPPORT = no
MTK_DISABLE_CAPABILITY_SWITCH = no
MTK_DISABLE_EFUSE = no
MTK_DISABLE_PERSIST_VSIM = no
MTK_DISABLE_POWER_ON_OFF_VOLTAGE_LIMITATION = no
MTK_DISPLAY_HIGH_RESOLUTION = no
MTK_DITHERING_SUPPORT = yes
MTK_DMNR_TUNING_AT_MD = no
MTK_DM_APP = no
MTK_DM_ENTRY_DISPLAY = no
MTK_DM_VERITY_OFF = yes
MTK_DOCOMO_SUPPORT = no
MTK_DP_FRAMEWORK = yes
MTK_DRE30_SUPPORT = no
MTK_DRM_APP = yes
MTK_DRM_KEY_MNG_SUPPORT = no
MTK_DRM_PLAYREADY_SUPPORT = no
MTK_DSPIRDBG = no
MTK_DTAG_DUAL_APN_SUPPORT = no
MTK_DTBO_FEATURE = yes
MTK_DUAL_CHARGER_SUPPORT = no
MTK_DUAL_DISPLAY_SUPPORT = no
MTK_DUAL_MIC_SUPPORT = no
MTK_DVFS_DISABLE_LOW_VOLTAGE_SUPPORT = no
MTK_DX_HDCP_SUPPORT = no
MTK_DYNAMIC_FPS_SUPPORT = no
MTK_DYNAMIC_SBP_SUPPORT = no
MTK_EAP_SIM_AKA = yes
MTK_EFUSE_WRITER_SUPPORT = no
MTK_EMBMS_SUPPORT = no
MTK_EMCAMERA_APP = yes
MTK_EMMC_DISCARD = no
MTK_EMMC_SUPPORT = yes
MTK_EMMC_SUPPORT_OTP = no
MTK_EMMC_SUPPORT_OTP_FOR_CUSTOMER = no
MTK_EMULATOR_SUPPORT = no
MTK_ENABLE_DIVX_DECODER = no
MTK_ENABLE_MD5 = no
MTK_ENABLE_NATIVE_ASAN = no
MTK_ENABLE_S263_DECODER = no
MTK_ENABLE_VIDEO_EDITOR = no
MTK_ENCRYPTION_DEFAULT_OFF = no
MTK_ENCRYPTION_FDE_TO_FBE = no
MTK_ENCRYPTION_TYPE_FILE = no
MTK_ENGINEERMODE_APP = yes
MTK_ENTITLEMENT_SUPPORT = no
MTK_EPDG_SUPPORT = no
MTK_ETHERNET_SUPPORT = no
MTK_ETWS_SUPPORT = yes
MTK_EXTERNAL_DONGLE_SUPPORT = no
MTK_EXTERNAL_LDO = no
MTK_EXTERNAL_SIM_ONLY_SLOTS = 0
MTK_EXTERNAL_SIM_SUPPORT = no
MTK_EXTMD_NATIVE_DOWNLOAD_SUPPORT = no
MTK_FACEBEAUTY_SUPPORT = yes
MTK_FACTORY_GAMMA_SUPPORT = no
#BSP : modify KALHBLEW-3 by di.qian 20180801 start
MTK_FACTORY_MODE_IN_GB2312 = no
#BSP : modify KALHBLEW-3 by di.qian 20180801 end
MTK_FACTORY_RESET_PROTECTION_SUPPORT = yes
MTK_FAN5402_SUPPORT = no
MTK_FAN5405_SUPPORT = no
MTK_FASTBOOT_SUPPORT = yes
MTK_FAT_ON_NAND = no
MTK_FD_FORCE_REL_SUPPORT = no
MTK_FD_SUPPORT = yes
MTK_FEMTO_CELL_SUPPORT = no
MTK_FENCE_SUPPORT = yes
MTK_FILEMANAGER_APP = yes
MTK_FIRST_MD = 1
MTK_FLIGHT_MODE_POWER_OFF_MD = yes
MTK_FLV_PLAYBACK_SUPPORT = yes
MTK_FMRADIO_APP = yes
MTK_FM_50KHZ_SUPPORT = no
MTK_FM_CHIP = MT6580_FM
MTK_FM_RX_AUDIO = FM_DIGITAL_INPUT
MTK_FM_RX_SUPPORT = yes
#BSP:modify BAEGHBLEQ-68 baihaiping 20190311 start
MTK_FM_SHORT_ANTENNA_SUPPORT = yes
TRAN_SHORT_ANTENNA_SUPPORT=yes
#BSP:modify BAEGHBLEQ-68 baihaiping 20190311 end
MTK_FM_SUPPORT = yes
MTK_FM_TX_AUDIO = FM_DIGITAL_OUTPUT
MTK_FM_TX_SUPPORT = no
MTK_FORCE_CLUSTER1 = no
MTK_FOTA_ENTRY = no
MTK_FOTA_SUPPORT = no
MTK_FSCK_MSDOS_MTK = no
MTK_FSCK_TUNE = no
MTK_FUSED_LOCATION_SUPPORT = no
MTK_GAS_SERVICE_SUPPORT = no
MTK_GAUGE_VERSION = 20
MTK_GLOBAL_PQ_SUPPORT = no
#PPD: modify by qianwen.guo for GMO optimization in 1G device ka6_GO 180810 start
MTK_GMO_RAM_OPTIMIZE = yes
MTK_GMO_ROM_OPTIMIZE = yes
#PPD: modify by qianwen.guo for GMO optimization in 1G device ka6_GO 180810 end
MTK_GOOGLEOTA_SUPPORT = no
MTK_GOOGLE_TRUSTY_SUPPORT = no
MTK_GPS_CHIP = MTK_GPS_MT6752
MTK_GPS_SUPPORT = yes
MTK_GPT_SCHEME_SUPPORT = yes
MTK_GPU_SUPPORT = yes
MTK_HAC_SUPPORT = no
# BSP:modify BAEGHBLEQ-28 by shuanglei.wang 20190301 start
MTK_HANDSFREE_DMNR_SUPPORT = no
# BSP:modify BAEGHBLEQ-28 by shuanglei.wang 20190301 end
MTK_HDMI_HDCP_SUPPORT = no
MTK_HDMI_SUPPORT = no
MTK_HDR_VIDEO_SUPPORT = no
MTK_HEADER_SUPPORT = no
MTK_HEADSET_ACTIVE_NOISE_CANCELLATION = no
MTK_HIGH_QUALITY_THUMBNAIL = yes
MTK_HIGH_RESOLUTION_AUDIO_SUPPORT = no
MTK_HW_ENHANCE = no
MTK_ICCS_SUPPORT = no
MTK_IMS_SUPPORT = no
MTK_INCALL_HANDSFREE_DMNR = no
MTK_INCALL_NORMAL_DMNR = no
MTK_INCLUDE_MODEM_DB_IN_IMAGE = yes
MTK_INTERNAL_HDMI_SUPPORT = no
MTK_INTERNAL_MHL_SUPPORT = no
MTK_INT_MD_SPE_FOR_EXT_MD = no
MTK_IN_HOUSE_TEE_SUPPORT = no
MTK_ION_SUPPORT = yes
MTK_IPTV_SUPPORT = no
MTK_IPV6_SUPPORT = yes
MTK_IPV6_TETHER_NDP_MODE = no
MTK_IPV6_TETHER_PD_MODE = no
MTK_IRTX_PWM_SUPPORT = no
MTK_IRTX_SUPPORT = no
MTK_JEITA_STANDARD_SUPPORT = no
MTK_JPEG_HW_RESIZER_TYPE = HW_RESIZER_TYPE_2
MTK_K64_SUPPORT=no
MTK_KERNEL_POWER_OFF_CHARGING = yes
MTK_LAUNCHER_UNREAD_SUPPORT = yes
MTK_LCM_DEVICE_TREE_SUPPORT = no
MTK_LCM_PHYSICAL_ROTATION = 0
#TDD:add for KBQJHBLEW-444 by yulichen 20181023 start
MTK_LIVEWALLPAPER_APP = no
#TDD:add for KBQJHBLEW-444 by yulichen 20181023 end
MTK_LOADER_BACKUP = yes
MTK_LOG2SERVER_APP = no
MTK_LOG2SERVER_INTERNAL = no
#TSD:add for KALHBLEW-1 multi md image by zhengli.gao 20180727 start
MTK_LOG_CUSTOMER_SUPPORT = yes
#TSD:add for KALHBLEW-1 multi md image by zhengli.gao 20180727 end
MTK_LOG_SUPPORT_GPS = yes
MTK_LOG_SUPPORT_MOBILE_LOG = yes
MTK_LOMO_SUPPORT = no
MTK_LWA_SUPPORT = no
MTK_M4U_SUPPORT = yes
MTK_MAGICONFERENCE_SUPPORT = no
MTK_MASS_STORAGE = yes
#TSD: BAEGHBLEQ-5 modified by yexiaochen at 20190219 start
MTK_MD1_SUPPORT = 3
#TSD: BAEGHBLEQ-5 modified by yexiaochen at 20190219 end
MTK_MD2_SUPPORT = 0
MTK_MD3_SUPPORT = 0
MTK_MD5_SUPPORT = 5
MTK_MDLOGGER_SUPPORT = yes
MTK_MDM_APP = no
MTK_MDM_FUMO = no
MTK_MDM_LAWMO = no
MTK_MDM_SCOMO = no
MTK_MD_DIRECT_TETHERING_SUPPORT = no
MTK_MD_LOW_BAT_SUPPORT = no
MTK_MD_SHUT_DOWN_NT = yes
MTK_MEMORY_COMPRESSION_SUPPORT = no
MTK_MERGE_INTERFACE_SUPPORT = no
MTK_META_RSSITRIGGER_SUPPORT = no
MTK_MIRAVISION_SETTING_SUPPORT = no
MTK_MKV_PLAYBACK_ENHANCEMENT = no
MTK_MLC_NAND_SUPPORT = no
#PPD: modify by qianwen.guo for Google requirement in 1G device ka6_GO 180810 start
MTK_MMPROFILE_SUPPORT = no
#PPD: modify by qianwen.guo for Google requirement in 1G device ka6_GO 180810 end
MTK_MMUMAP_SUPPORT = no
MTK_MOBILE_MANAGEMENT = yes
MTK_MODEM_MONITOR_SUPPORT = yes
MTK_MP2_PLAYBACK_SUPPORT = no
MTK_MPEG_DASH_SUPPORT = no
MTK_MSDC1_NOT_SUPPORT_SDR104 = no
MTK_MT6306_SUPPORT = no
MTK_MT6333_SUPPORT = no
MTK_MT8193_HDCP_SUPPORT = no
MTK_MT8193_HDMI_SUPPORT = no
MTK_MT8193_SUPPORT = no
MTK_MTKLOGGER_SUPPORT = yes
MTK_MTKPS_PLAYBACK_SUPPORT = no
MTK_MULTIBRIDGE_SUPPORT = no
MTK_MULTI_PARTITION_MOUNT_ONLY_SUPPORT = no
MTK_MULTI_SIM_SUPPORT = dsds
MTK_MULTI_STORAGE_SUPPORT = yes
MTK_MULTI_WINDOW_SUPPORT = no
MTK_MUX_CHANNEL = 32
MTK_NAND_MTK_FTL_SUPPORT = no
MTK_NAND_PAGE_SIZE = 2K
MTK_NAND_UBIFS_SUPPORT = no
MTK_NATIVE_FENCE_SUPPORT = no
MTK_NEON_SUPPORT = yes
MTK_NETWORK_LOG_SUPPORT = yes
MTK_NETWORK_TYPE_ALWAYS_ON = no
MTK_NETWORK_TYPE_DISPLAY = no
MTK_NET_RFC7217 = no
MTK_NEW_COMBO_EMMC_SUPPORT = yes
MTK_NEW_IPTABLES_SUPPORT = yes
MTK_NFC_ADDON_SUPPORT = no
MTK_NFC_GSMA_SUPPORT = no
MTK_NFC_HCE_SUPPORT = no
MTK_NFC_MT6605 = no
MTK_NFC_OMAAC_SUPPORT = yes
MTK_NFC_PACKAGE = MTK
MTK_NFC_SE_NUM = 7
MTK_NFC_SUPPORT = no
MTK_NO_NEED_USB_LED = no
MTK_NTFS_OPENSOURCE_SUPPORT = no
MTK_NUM_MODEM_PROTOCOL = 2
MTK_NVCONFIG_PARTITION_SUPPORT = no
MTK_NVRAM_SECURITY = no
MTK_OD_SUPPORT = no
MTK_OGG_RECORD_SUPPORT = no
MTK_OGM_PLAYBACK_SUPPORT = no
MTK_OMACP_SUPPORT = yes
MTK_OMADRM_SUPPORT = yes
MTK_OMA_DOWNLOAD_SUPPORT = yes
MTK_ONLY_OWNER_SIM_SUPPORT = no
MTK_OVERLAY_ENGINE_SUPPORT = no
MTK_OWNER_SDCARD_ONLY_SUPPORT = no
MTK_PARTITION_TABLE_PLAIN_TEXT = no
MTK_PASSPOINT_R2_SUPPORT = no
MTK_PCA9575A_SUPPORT = no
MTK_PCM_RECORD_SUPPORT = no
MTK_PERFSERVICE_SUPPORT = yes
MTK_PERMISSION_CONTROL = yes
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 start
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 end
MTK_PHONE_VT_VOICE_ANSWER = no
MTK_PLATFORM = MT6580
MTK_PLATFORM_OPTIMIZE = yes
MTK_PLAYBACK_60FPS_TO_30FPS = no
MTK_PMIC_MT6397 = no
MTK_PM_STATS_SUPPORT = no
MTK_POST_PROCESS_FRAMEWORK_SUPPORT = no
MTK_POWER_EXT_DETECT = no
MTK_PQ_COLOR_MODE = DISP
MTK_PQ_SUPPORT = PQ_HW_VER_2
MTK_PRIVACY_PROTECTION_LOCK = yes
MTK_PRODUCT_INFO_SUPPORT = yes
MTK_PROTOCOL1_RAT_CONFIG = W/G
#TSD: BAEGHBLEQ-5 modified by yexiaochen at 20190219 start
MTK_PROTOCOL2_RAT_CONFIG = W/G
#TSD: BAEGHBLEQ-5 modified by yexiaochen at 20190219 end
MTK_PROTOCOL3_RAT_CONFIG = G
MTK_PUMP_EXPRESS_PLUS_30_SUPPORT = no
MTK_PUMP_EXPRESS_SUPPORT = no
MTK_RADIOOFF_POWER_OFF_MD = no
MTK_RAT_BALANCING = no
MTK_RAT_WCDMA_PREFERRED = yes
MTK_RAWFS_NAND_SUPPORT = no
MTK_RCS_SUPPORT = no
MTK_REGIONALPHONE_SUPPORT = no
MTK_RELEASE_PACKAGE = rel_customer_basic rel_customer_kernel_3-18 rel_customer_modem
MTK_RESOLUTION_SWITCH_SUPPORT = no
MTK_RESOURCE_OPTIMIZATION =
MTK_RILD_READ_IMSI = yes
MTK_RIL_MODE = c6m_3rild
MTK_ROTATION_OFFSET_SUPPORT = no
MTK_RSDM_APP = no
MTK_RTP_OVER_RTSP_SUPPORT = yes
#PPD: modified ProcessManager by hanhao 20180809 start
#MTK_RUNNING_BOOSTER_DEFAULT_ON = yes
#MTK_RUNNING_BOOSTER_SUPPORT = yes
#PPD: modified ProcessManager by hanhao 20180809 end
MTK_RUNNING_BOOSTER_UPGRADE = no
MTK_SCOMO_ENTRY = no
MTK_SCREEN_OFF_WIFI_OFF = no
MTK_SDIOAUTOK_SUPPORT = no
MTK_SEARCH_DB_SUPPORT = yes
MTK_SEC_MODEM_AUTH = no
MTK_SEC_MODEM_ENCODE = no
MTK_SEC_MODEM_NVRAM_ANTI_CLONE = yes
MTK_SEC_SECRO_AC_SUPPORT = yes
MTK_SEC_VIDEO_PATH_SUPPORT = no
MTK_SEC_WFD_VIDEO_PATH_SUPPORT = no
MTK_SEND_RR_SUPPORT = yes
MTK_SENSOR_BIO = no
MTK_SENSOR_SUPPORT = yes
MTK_SHARED_SDCARD = yes
MTK_SHOW_MSENSOR_TOAST_SUPPORT = no
#SID1:modify BAEGHBLEQ-61 by weike.xu 20190311 start
MTK_SIGNATURE_CUSTOMIZATION = yes
#SID1:modify BAEGHBLEQ-61 by weike.xu 20190311 end
MTK_SIM1_SOCKET_TYPE = 1
MTK_SIM2_SOCKET_TYPE = 1
MTK_SIM_HOT_SWAP = yes
MTK_SIM_HOT_SWAP_COMMON_SLOT = no
MTK_SIM_RECOVERY = yes
MTK_SINGLE_3DSHOT_SUPPORT = no
MTK_SINGLE_IMEI = no
MTK_SLOW_MOTION_VIDEO_SUPPORT = no
MTK_SMARTBOOK_SUPPORT = no
MTK_SMARTSWITCH_SUPPORT = no
MTK_SMSREG_APP = no
MTK_SMT_SUPPORT = no
MTK_SOTER_SUPPORT = no
MTK_SPECIFIC_SM_CAUSE = no
MTK_STREAMING_VIDEO_SUPPORT = no
MTK_SUBTITLE_SUPPORT = no
MTK_SUPER_DIMMING_SUPPORT = no
MTK_SWABF_SUPPORT = no
MTK_SWCHR_SUPPORT = no
MTK_SYSTEM_UPDATE_SUPPORT = no
MTK_TABLET_PLATFORM = no
MTK_TABLET_PLUGIN_BUILD = no
MTK_TABLET_TS_ENHANCEMENT = no
MTK_TB_APP_LANDSCAPE_SUPPORT = no
MTK_TC1_FEATURE = no
MTK_TC9_FEATURE = no
MTK_TDD_DATA_ONLY_SUPPORT = no
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 start
#BSP:modify KALHBLEW-13 by xiaopan.ji 20180801 end
MTK_TELEPHONY_BOOTUP_MODE_SLOT1 = 0
MTK_TELEPHONY_BOOTUP_MODE_SLOT2 = 1
MTK_TELEPHONY_CONN_LOG_CTRL_SUPPORT = yes
MTK_TELEPHONY_FEATURE_SWITCH_DYNAMICALLY = no
MTK_TELEPHONY_MODE = 0
MTK_TER_SERVICE = no
MTK_TETHERINGIPV6_SUPPORT = no
MTK_TETHERING_EEM_SUPPORT = no
MTK_THERMAL_PA_VIA_ATCMD = yes
MTK_TINYSYS_SCP_SUPPORT = no
MTK_TINY_UTIL = no
MTK_TOUCH_BOOST = no
MTK_TTY_SUPPORT = yes
#BSP: modify KBQJHBLEW-679 by yinpeng.zhang 20190114 start
MTK_UART_USB_SWITCH = yes
#BSP: modify KBQJHBLEW-679 by yinpeng.zhang 20190114 end
MTK_UFS_BOOTING = no
MTK_UFS_OTP = no
MTK_ULTRA_DIMMING_SUPPORT = no
MTK_USB_PHONECALL = NONE
MTK_USER_ROOT_SWITCH = no
MTK_USER_SPACE_DEBUG_FW = yes
MTK_USE_ANDROID_MM_DEFAULT_CODE = no
MTK_USE_RESERVED_EXT_MEM = no
MTK_USSI_SUPPORT = no
MTK_VIDEO_4KH264_SUPPORT = no
MTK_VIDEO_HEVC_SUPPORT = no
MTK_VIDEO_VP8ENC_SUPPORT = no
MTK_VILTE_SUPPORT = no
MTK_VIWIFI_SUPPORT = no
MTK_VOICE_UNLOCK_USE_TAB_LIB = no
MTK_VOIP_ENHANCEMENT_SUPPORT = yes
MTK_VOIP_HANDSFREE_DMNR = no
MTK_VOIP_NORMAL_DMNR = no
MTK_VOLTE_SUPPORT = no
MTK_VOW_SUPPORT = no
MTK_VPU_SUPPORT = no
MTK_VR_HIGH_PERFORMANCE_SUPPORT = no
MTK_VSS_SUPPORT = no
MTK_VT3G324M_SUPPORT = no
MTK_WAPI_SUPPORT = no
MTK_WAPPUSH_SUPPORT = yes
MTK_WB_SPEECH_SUPPORT = yes
MTK_WEEK_NO = W18.51
MTK_WFC_SUPPORT = no
MTK_WFD_HDCP_RX_SVP_SUPPORT = no
MTK_WFD_HDCP_TX_SUPPORT = no
MTK_WFD_SINK_SUPPORT = no
MTK_WFD_SINK_UIBC_SUPPORT = no
MTK_WFD_SUPPORT = no
MTK_WIFI_P2P_SUPPORT = yes
MTK_WIRELESS_CHARGER_SUPPORT = no
MTK_WLANBT_SINGLEANT = no
MTK_WLAN_CHIP = CONSYS_MT6735
MTK_WLAN_PATH_SET = siso
MTK_WLAN_SUPPORT = yes
MTK_WMA_PLAYBACK_SUPPORT = no
MTK_WMV_PLAYBACK_SUPPORT = yes
MTK_WPA2PSK_SUPPORT = no
MTK_WVDRM_L1_SUPPORT = no
MTK_WVDRM_SUPPORT = yes
MTK_YAML_SCATTER_FILE_SUPPORT = yes
NAND_OTP_SUPPORT = no
NXP_SMARTPA_SUPPORT = no_nxp
OP01_COMPATIBLE = no
OPTR_SPEC_SEG_DEF = NONE
PURE_AP_USE_EXTERNAL_MODEM = no
SENSOR_BATCH_SUPPORT = no
SIM_ME_LOCK_MODE = 0
SIM_REFRESH_RESET_BY_MODEM = yes
TARGET_COPY_OUT_ODM = no
USE_FRAUNHOFER_AAC = no
USE_XML_AUDIO_POLICY_CONF = 1
WIFI_WEP_KEY_ID_SET = no
MTK_UCE_SUPPORT = no
MTK_THUMBNAIL_OPTIMIZATION = yes
MTK_O2_ENTITLEMENT_SUPPORT = no
MTK_UT_XCAP_SUPPORT = yes
MTK_HIDL_PROCESS_CONSOLIDATION_ENABLED = yes
MTK_OVERRIDES_APKS = yes
MTK_QOS_SUPPORT = no
MTK_DIGITS_SUPPORT = no
MTK_DEFAULT_WRITE_DISK = no
MTK_HIFIAUDIO_SUPPORT = no
MTK_ANDROID_FOR_WORK_SUPPORT = no
MTK_SENSORS_1_0 = yes
MTK_SWIP_WMAPRO = no
MTK_NLP_SUPPORT = yes
MTK_BT_AVRCP_TG_APP_SETTINGS_SUPPORT = yes
MTK_EXCHANGE_SUPPORT = yes
MTK_RUNNING_BOOSTER_THROTTLE = yes
# add by LRZ for Project macro configuration 20171110
CUSTOM_TRAN_PROJECT= tran_ba2
CUSTOM_TRAN_BOM= tran_ba2_a1
# add end
#Infinix:by gujiacheng 20171216 start
TRAN_BOOMPLAYER_UNINSTALL_SUPPORT = yes
#Infinix:by gujiacheng 20171216 end
#BSP:add KALHBLEW-2 by haiping.bai 20180720 start
TRAN_AUDIOTEST_SUPPORT = yes
#BSP:add KALHBLEW-2 by haiping.bai 20180720 end
#TSD:add for KALHBLEW-1 multi md image by zhengli.gao 20180726 start
TRAN_MODEM_MULTI_MD_IMAGE_SUPPORT = yes
#TSD:add for KALHBLEW-1 multi md image by zhengli.gao 20180726 end
#BSP:add CFQHLYE-14 by hong.zhang for porting f2fs 201800725 start
TRAN_USE_F2FS = yes
#BSP:add CFQHLYE-14 by hong.zhang for porting f2fs 20180725 end
# BSP: Added KALHBLEW-3 by xiaolin.wu 20180813 start
TRAN_SEC_FILE_COPY_SUPPORT=yes
# BSP: Added KALHBLEW-3 by xiaolin.wu 20180813 end
#BSP:modify BAEGHBLEQ-792 by yong.jiang 20190711 start
TRAN_FINGERPRINT_SUPPORT = sunwave_tee microarray_tee cdfinger_tee focaltech_tee
#BSP:modify BAEGHBLEQ-792 by yong.jiang 20190711 end
#TDD:add by huangweijia for XWWJHBLYWO-232 on 2018.07.09 start
RLK_INFINIX_AGINGFUNCTION=yes
TRAN_AGFUNCTION_FM_SUPPORT=yes
#TDD:add by huangweijia for XWWJHBLYWO-232 on 2018.07.09 end
#TDD:add XLLJHBLEW-430 by weijia.huang on 2018.08.24 start
TRAN_BREATH_LIGHT_SUPPORT = no
TRAN_NOTIFICATION_LIGHT_SUPPORT = no
#TDD:add XLLJHBLEW-430 by weijia.huang on 2018.08.24 end
#PPD: add KALHBLEW-16 by peng.zhang 20180806 start
TRAN_VM_STARTSIZE = 8m
TRAN_VM_MINFREE_SIZE = 2m
TRAN_VM_MAXFREE_SIZE = 8m
TRAN_RM_ACTIVITY_SUPPORT = yes
#PPD: add KALHBLEW-16 by peng.zhang 20180806 end
#TDD:add AWESLITE-16 by bo.zhou 2018-7-26 start
BUILD_GMS=yes
BUILD_AGO_GMS = yes
TRANSSION_CLIENT_ID = android-transsion-tecno-rev1
TRAN_GMSEXPRESS_PLUS_SUPPORT = yes
#TDD:add AWESLITE-16 by bo.zhou 2018-7-26 end
#SID:add BFBLYB-16 by lilonglong 20180717 start
RLK_PROJECT = tecno_ba2
OS_PRODUCT_VERSION = HIOS4.1
OS_GO_SUPPORT = yes
OS_NOTCH_SUPPORT = no
TRANSSION_PRODUCT_DEVICE = TECNO-BA2
TRANSSION_PRODUCT_NAME = H8027
TRANSSION_DEVICE_NAME = TECNO POP 2 Plus
TRANSSION_PRODUCT_MODEL = TECNO BA2
TRANSSION_BRAND_NAME = TECNO
TRANSSION_PRODUCT_MANUFACTURER = TECNO MOBILE LIMITED
TRANSSION_PRODUCT_BOARD = TECNO-BA2
OS_MYPICTURE_SUPPORT = no
TRAN_OS_CALCULATOR_SUPPORT = yes
OS_APPS_UNINSTALL_SUPORRT = yes
OS_SINGLE_HAND_SUPPORT = yes
TRAN_RCS_GO_SUPPORT = yes
TRAN_SYS_STASL = yes
TRAN_FLIPKART_DELETE = yes
TRAN_DUALAIDVEND_DELETE = yes
#SID:modify BAEGHBLEQ-762 by gege.huang 20190627 start
TRAN_BOOMPLAY_SUPPORT = no
#SID:modify BAEGHBLEQ-762 by gege.huang 20190627 end
TRAN_SPORTYBET_SUPPORT = no
TRANSSION_REMOVE_ASOP = yes
#SID:modify BAEGHBLEQ-696 by weike.xu 20190521 start
TRAN_ODEX_SUPPORT_LEV1 = yes
#SID:modify BAEGHBLEQ-696 by weike.xu 20190521 end
#APP:Mod by xian.zhang for KBQJHBLEW-750 20190111 start
THIRD_FOOVIEW_SUPPORT = no
#APP:Mod by xian.zhang for KBQJHBLEW-750 20190111 end
#SID:modify BAEGHBLEQ-762 by gege.huang 20190627 start
TRANSSION_WHATSAPP_SUPPORT = no
#SID:modify BAEGHBLEQ-762 by gege.huang 20190627 end
#APP: Mod KBQJHBLEW-1092 by zhongyuan.li 20190128 start
TRAN_MULTIACCOUNT_SUPPORT = no
#APP: Mod KBQJHBLEW-1092 by zhongyuan.li 20190128 end
#APP:Mod by xian.zhang for KBQJHBLEW-827 20190123 start
TRAN_KIKA_SUPPORT = no
TRAN_KIKA_GO_SUPPORT = no
#APP:Mod by xian.zhang for KBQJHBLEW-827 20190123 end
#APP: Modify by zhongyuan.li for BAEGHBLEQ-81 20190314 start
TRAN_TENCOSPOT_SUPPORT = no
TRAN_PACKSELL_SUPPORT = no
#APP: Modify by zhongyuan.li for BAEGHBLEQ-81 20190314 end
#APP:Mod by xian.zhang for KBQJHBLEW-750 20190111 start
TRANSSION_EAGLEEE_DELETE = no
#APP:Mod by xian.zhang for KBQJHBLEW-750 20190111 end
TRANSSION_BUILD_GO = yes
BUILD_GO_GMS = yes
#APP: Mod KBQJHBLEW-730 by xian.zhang 20190114 start
TRAN_PLAMSTORE_SUPPORT = no
#APP: Mod KBQJHBLEW-730 by xian.zhang 20190114 end
TRAN_FACEID_SUPPORT = yes
#APP: Modify by zhongyuan.li for BAEGHBLEQ-129 20190321 start
#TRANSSION_YOUTUBE_SUPPORT = no
#APP: Modify by zhongyuan.li for BAEGHBLEQ-129 20190321 end
#SID:add BFBLYB-16 by lilonglong 20180717 end
#SID2:modify BAEGHBLEQ-61 by weike.xu 20190311 start
TRAN_DISABLE_ADB_RSA_SUPPORT = no
TRAN_GMS_MUSIC_UNINSTALL = no
OS_NOTEBOOK_SUPPORT = no
#SID:modify BAEGHBLEQ-341 by weike.xu 20190328 start
OS_WEATHER_SUPPORT = no
#SID:modify BAEGHBLEQ-341 by weike.xu 20190328 end
TRAN_PLAMSTORE_SUPPORT = yes
#APP: Modify by zhongyuan.li for BAEGHBLEQ-113 20190321 start
TRAN_RECHARGEKING_SUPPORT = no
#APP: Modify by zhongyuan.li for BAEGHBLEQ-113 20190321 end
#APP: Modify by mingliang.cao for BAEGHBLEQ-342 20190403 start
TRAN_OPERANEWS_SUPPORT = no
#APP: Modify by mingliang.cao for BAEGHBLEQ-342 20190403 end
TRAN_TRANSFER_SUPPORT = no
#APP: Modify by zhongyuan.li for BAEGHBLEQ-81 20190314 start
OS_XSHARE_SUPPORT = no
#APP: Modify by zhongyuan.li for BAEGHBLEQ-81 20190314 end
#SID2:modify BAEGHBLEQ-61 by weike.xu 20190311 end
#BSP: add KALHBLEW-10 by wang.qin 20180806 start
TRAN_FT_CUSTOM_PERMISSION_SUPPORT = yes
TRAN_FT_RESET = yes
TRAN_FT_REBOOT = yes
TRAN_FT_CUSTOM_SHELL = yes
TRAN_FT_BENCHFLAG_SUPPORT = yes
#BSP: add KALHBLEW-10 by wang.qin 20180806 end

#BSP:add BAEGHBLEQ-53  by ming.chen 20190306 start
TRAN_CHARGER_LEDS_SUPPORT = no
#BSP:add BAEGHBLEQ-53  by ming.chen 20190306 end

#TDD: add by shouhui.li Midtest3.0 start
RLK_MIDTEST_SUPPORT = yes
TRAN_TECNO_MIDTEST_NEW_SUPPORTED = yes
TRAN_TECNO_MIDTEST_SUPPORTED = yes
TRAN_MIDTEST_SUPPORT = yes
TRAN_MIDTEST_CLEARPA_SUPPORTED = no
TRAN_MIDTEST_BCAMERA_AF_SUPPORTED = yes
TRAN_MIDTEST_DUAL_CAMERA_SUPPORTED = no
TRAN_MIDTEST_GYRO_SENSOR_SUPPORTED = no
TRAN_MIDTEST_DISTANCE_SENSOR_SUPPORTED = yes
TRAN_MIDTEST_LIGHT_SENSOR_SUPPORTED = yes
TRAN_MIDTEST_COMPASS_SUPPORTED = no
TRAN_MIDTEST_FLASH_FORWARD_SUPPORTED = yes
TRAN_MIDTEST_TRICOLOR_LEDS_SUPPORTED = no
TRAN_MIDTEST_DUAL_MIC_SUPPORTED = no
TRAN_MIDTEST_HALL_SUPPORTED = no
TRAN_MIDTEST_CAMERAKEY_SUPPORTED = no
TRAN_MIDTEST_FINGERPRINTF_SUPPORTED = yes
TRAN_MIDTEST_OTG_NO_SUPPORTED = yes
TRAN_MIDTEST_BUTTON_LEDS_SUPPORTED = no
TRAN_MIDTEST_INDICATOR_LIGHT_SUPPORTED = no
TRAN_MIDTEST_DUAL_CAMERA_WESTALGO = no
TRAN_MIDTEST_FINGER_SW9362_SUPPORTED = no
MIDTEST_FILL_LIGHT_SUPPORTED = no
MIDTEST_FINGER_SW9088_SUPPORTED = yes
MIDTEST_FINGER_CD_TEE_SUPPORTED = no
#TDD: add by shouhui.li Midtest3.0 end

#BSP: add XLLSHLSS-3114 by peixuan.qiu 20180808 start
TRAN_TKV_SUPPORTED = yes
#BSP: add XLLSHLSS-3114 by peixuan.qiu 20180808 end

#TSD: modify BAEGHBLEQ-98 by gu.haiyuan 20190403 start
OS_INCOMING_FLASHLIGHT_SUPPORT = no
#TSD: modify BAEGHBLEQ-98 by gu.haiyuan 20190403 end

#*********************CDD Feature Only Start******************************#
#CameraAP:add CAMERAEYL-288 shiyang.zhang 2018-08-14 start
HIOS_CAMERA_DEBUG_SUPPORT = yes
#TECNO_CAMERA_PORTRAITURE_SUPPORT = yes
HIOS_CAMERA_FRONT_FLASH_TORCH_ADJUSTMENT_SUPPORT = yes
HIOS_CAMERA_AUTO_WATERMARK_SUPPORT = yes
HIOS_CAMERA_SCREENFLASH_SUPPORT = yes
TECNO_CAMERA_FRONT_MIRROR = yes
#TECNO_CAMERA_CONTINUOUSSHOT_SUPPORT = yes
HIOS_CAMERA_FRONT_PHOTO_FLASH_ON_SUPPORT = yes
TECNO_CAMERA_FULLSCREEN_SUPPORT = yes
HIOS_CAMERA_DISABLE_ZSD = yes
TRAN_CAMERA_FRONT_PORTRAITURE_STBlUR_SUPPORT = yes
#TECNOCAM_FALSEDOUBLECAMERA = yes
HIOS_CAMERA_FRONT_VIDEO_FLASH_AUTO_NO_SUPPORT = yes
TRANSSION_QUICKCAPTURE_SUPPORT = false
HIOS_CAMERA_USE_GOOGLE_PHOTOS_SUPPORT = yes
TECNO_CAMERA_NIGHT_SCENE_SUPPORT = yes
#CameraAP:add CAMERAEYL-288 shiyang.zhang 2018-08-14 end
#*********************CDD Feature Only End******************************#
#BSP : add KBQJHBLEW-7 by di.qian 20180919 start
TRAN_RECOVERY_NOTCH_SUPPORT = yes
#BSP : add KBQJHBLEW-7 by di.qian 20180919 end

#*********************PPD Feature Only Start******************************#
#PPD: add ProcessManager by hanhao 20180809 start
TRAN_PROCMGR_SUPPORT=yes
TRAN_PROCMGR_MODE=deep
TRAN_LMK_BOOST_SUPPORT=yes
#PPD: add ProcessManager by hanhao 20180809 end
#PPD: support onekeyclean by shasha.liu 20180913 start
TRAN_ONEKEYCLEAN_SUPPORT=yes
#PPD: support onekeyclean by shasha.liu 20180913 end
#PPD add by zeyang.li for pnp feature at 20180801 start
TRAN_PNPMGR_SUPPORT = yes
#PPD add by zeyang.li for pnp feature at 20180801 end
#PPD: add XLLSHLSS-2214 by ruishuang.jia 18/06/20 start
TRAN_PPD_LUCID_DISABLED = no
#PPD: add XLLSHLSS-2214 by ruishuang.jia 18/06/20 end
#*********************PPD Feature Only End******************************#
#SID: add BFBLYB-355 jinbao 20180727 start
TRAN_COSOFTWARE_SUPPORT=yes
TRAN_COSOFTWARE_GROUP_NAME=ba2_h8027_go_efuse
#SID: add BFBLYB-355 jinbao 20180727 end

#APP: add XLESHBLEL-226 by hanxin.zhang 2018/9/29 start
TRAN_DEFAULT_PHOTO_SUPPORT = yes
#APP: add XLESHBLEL-226 by hanxin.zhang 201809/29 end

#TSD add XLESHBLEL-171 by chenchen 2018.10.09 start
TRAN_RCS_SUPPORT = yes
#TSD add XLESHBLEL-171 by chenchen 2018.10.09 end

#APP: add XLLJHBLEW-774 by zhangxu 20181011 start
TRAN_AVOID_WIFI_ATTENUATION_SUPPORT = yes
#APP: add XLLJHBLEW-774 by zhangxu 20181011 end

#TDD:add XLESHBLEL-628 by wenqiang.zhang 20181015 start
PRODUCT_PROPERTY_OVERRIDES += ro.logd.size=262144
#TDD:add XLESHBLEL-628 by wenqiang.zhang 20181015 end

#TDD:add BEFBLYQ-521 by bo.zhou 20181018 start
TRAN_GMSEXPRESS_PLUS_SUPPORT=yes
#TDD:add BEFBLYQ-521 by bo.zhou 20181018 end

#APP: add lizhao for LBLHBLEW-705 20181229 start
#App: modify BAEGHBLEQ-371 by xu.tang 2019.04.01 start
TRAN_FP_SLIDE_ENROLL_SUPPORT = no
#App: modify BAEGHBLEQ-371 by xu.tang 2019.04.01 end
#APP: add lizhao for LBLHBLEW-705 20181229 end

#APP: Add by fuyu for KBQJHBLEW-650 20181102 start
TRAN_CHROMECUSTOMIZATION_SUPPORT = yes
#APP: Add by fuyu for KBQJHBLEW-650 20181102 end

#APP: add KBQJHBLEW-117 by hanxin.zhang 2019/1/8 start
TRAN_OOBE_SCREENSHOT_NOT_SUPPORT = yes
#APP: add KBQJHBLEW-117 by hanxin.zhang 2019/1/8 end

#APP: add KBQJHBLEW-708 by zhangxu 20190122 start
INDIA_DOZE_WHATSAPP_MODE_SUPPORT = no
#APP: add KBQJHBLEW-708 by zhangxu 20190122 end

#TDD:add KBQJHBLEW-1139 by wenqiang.zhang 20190212 start
PRODUCT_PROPERTY_OVERRIDES += ro.product.first_api_level=27
#TDD:add KBQJHBLEW-1139 by wenqiang.zhang 20190212 end

#APP:add BAEGHBLEQ-76 by xu.tang 20190313 start
RLK_MULTI_TOUCH_SUPPORT = no
#APP:add BAEGHBLEQ-76 by xu.tang 20190313 end

#APP:add BAEGHBLEQ-87 by yanjie.wang 20190325 start
CUSTOM_TRAN_PROJECT=tran_ba2
#APP:add BAEGHBLEQ-87 by yanjie.wang 20190325 end

#APP:add BAEGHBLEQ-489 by gege.huang 20190411 start
TRAN_PALMSTORE_UNINSTALL_SUPPORT = yes
#APP:add BAEGHBLEQ-489 by gege.huang 20190411 end

#APP:add BAEGHBLEQ-629 by zhaoyang.shi 20190605 start
TRAN_MANUAL_GUIDE_SUPPORT = TECNO-BA2
#APP:add BAEGHBLEQ-629 by zhaoyang.shi 20190605 start

#SID:add BAEGHBLEQ-716 by gege.huang 20190611 start
TRAN_XSHARE_SUPPORT = no
TRAN_AHA_GO_SUPPORT = yes
TRAN_AHA_UNINSTALL_SUPPORT = yes
TRAN_TOUCHPAL_SUPPORT = no
#SID:add BAEGHBLEQ-716 by gege.huang 20190611 end

#SID:add BAEGHBLEQ-762 by gege.huang 20190627 start
TRAN_TOUCHPAL_PINYIN_SUPPORT = no
#SID:add BAEGHBLEQ-762 by gege.huang 20190627 end

#SID:add BAEGHBLEQ-812 by gege.huang 20190807 start
TRAN_ASPHALT_GO_SUPPORT = yes
#SID:add BAEGHBLEQ-812 by gege.huang 20190807 end

#SDD: add ota clear wallpaper by li.li start 20190929 start
TRAN_OTA_CLEAR_WALLPAPE_SUPPORT = no
#SDD: add ota clear wallpaper by li.li start 20190929 end

PLATFORM_BASE_OS=TECNO/H8027/TECNO-BA2:8.1.0/O11019/ABC-210119V515:user/release-keys
MTK_SECURITY_SW_SUPPORT=yes
TRAN_FT_MEM_TRACE_SUPPORT=yes
#BSP:add XLYLHLWY-4161 by wang.qin for dramtest properties 20190624 start
ifeq ($(strip $(TRAN_FT_MEM_TRACE_SUPPORT)), yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.proinfo.memory.trace.support.dramtest=yes
  PRODUCT_PROPERTY_OVERRIDES += ro.proinfo.memory.trace.support.kene=yes
endif
#BSP:add XLYLHLWY-4161 by wang.qin for dramtest properties 20190624 end
MTK_PERSIST_PARTITION_SUPPORT = yes
MTK_ATF_SUPPORT = no
MTK_TEE_SUPPORT = yes
MICROTRUST_TEE_SUPPORT = yes
TRUSTONIC_TEE_SUPPORT = no
MTK_TEE_GP_SUPPORT = no
